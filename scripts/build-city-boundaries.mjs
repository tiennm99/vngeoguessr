...
/**
 * Rewrite the barrel module that imports every boundary that exists.
 *
 * Same reason as the panorama indexes: static imports let the data be bundled
 * with the server build, but a city that has not been generated yet would then
 * be a hard build error. Generating the list from disk keeps a partial build
 * usable.
...
for (const [code, config] of Object.entries(CITIES)) {
  ...
  // Rewritten after each city so an interrupted run leaves a consistent barrel.
  writeBarrel();
}
