...
function writeBarrel() {
  ...
  const entries = files.map((name) => ({
    ...
    // filename but not in a bare identifier or an unquoted object key. Emitting
    // them raw makes the generated barrel a syntax error.
    ident: name.replace(/\.json$/, '').replace(/[^a-z0-9]/gi, '_'),
    file: name,
  }));
